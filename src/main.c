...
int Menu(int64_t *konto) {
    ...
    printf("\nDein Guthaben: %lld$\n", *konto);
    printf("Was möchtest du tun? (Betteln, Blackjack, Roulette, Poker, Settings, Exit)\n> ");
    ...
    switch (cmd) {
        ...
        case EXIT:
            ...
            return 0;
        case UNKNOWN:
            printf("Unbekannter Befehl! Bitte achte auf Groß/Kleinschreibung.\n");
            break;
        default:
...
void SaveGameBinary(int64_t konto) {
    ...
    if (file) {
        ...
        fclose(file);
        printf("\n[System] Kontostand (%lld) binär gesichert.\n", konto);
    } else {
